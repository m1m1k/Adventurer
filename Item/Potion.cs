...
namespace TimeLords
{
    public class Potion : Item
    {
        ...
        public override Level Eat(Level currentLevel, Creature consumer)
        {
			...
			currentLevel.creatureList[creatureIndex].Affect(eatEffect, currentLevel); //Apply effect to the consumer

            currentLevel.creatureList[creatureIndex].inventory.Remove(this); //It's been drunk, it no longer exists; in the future, rewrite to transfer contents to creature's stomach, and leave behind an empty container in inventory
            return currentLevel;
        }
